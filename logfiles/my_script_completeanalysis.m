...
%cd session28_July_2015_07_04_23_PM/
%cd session30_July_2015_11_40_57_AM/
%cd session30_July_2015_04_28_14_PM/
%cd session31_July_2015_05_39_28_PM/
%cd session03_August_2015_03_15_46_PM/
%cd session03_August_2015_03_29_25_PM/
%cd session03_August_2015_04_47_23_PM/
%cd session03_August_2015_06_48_50_PM/
%cd session04_August_2015_02_11_06_PM/
%cd session07_August_2015_08_00_49_PM/
%cd session11_August_2015_04_28_17_PM/
%cd session11_August_2015_04_57_22_PM/
%cd session12_August_2015_01_32_35_PM/ %Looks OK Manual
...
%     imu.data(:,2) = imu.data(:,2) +(vrpnrpy(1,1) - imu.data(1,2));
%     imu.data(:,3) = imu.data(:,3) +(vrpnrpy(1,2) - imu.data(1,3));
%     imu.data(:,4) = imu.data(:,4) +(vrpnrpy(1,3) - imu.data(1,4));
...
cd ..
%% Plot IMU Diff:
matching_vrpntime = matchnearest(imu.data(:,1), vrpn.data(:,1));
imudiff = vrpnrpy(matching_vrpntime,:) - imu.data(:,2:4);
figure; plot(imutime, imudiff*(180/pi));
...
while ((count < 20)&&~stop_loop)%Almost like while(1) loop
   ...
   % freq_bode2 = freq2(inds_bode2);%To get the exact value Not needed here
    ...
    U = transpose(Vrpnrpyfreqdata(inds_bode2));
...
figure(6),subplot(2,1,1), plot(freq_bode2,abs(Ymean),'b*-');
subplot(2,1,2), plot(freq_bode2,abs(Umean),'b*-');
Ghat = Ymean./Umean; %Transfer Function Response
figure(7), semilogx(freq_bode2,20*log10(abs(Ymean./Umean)),'bo-');
...
figure(8),semilogx(freq_bode2,Gvar,'r'),xlabel('freq(Hz)'),ylabel('Variance(G) for the current expt');
...
modelnumber = 9; paramcount = 3;%thetabest = [0.3009;-0.0230; 0.0690]; MLE 5.6401
...
for count = 1:50
    ...
    [thetastar,fval] = fminsearch(@(x) Transferfunction(x,freq_bode2',Ghat,modelnumber),theta0,options);
    if fval < fvalmin
        fvalmin = fval;
        thetamin = thetastar;
    end
...
[H,W] = bode(G1,freq_bode2);
H = squeeze(H);
W = rem(squeeze(W)*pi/180,2*pi);
W(W>pi) = W(W>pi) - 2*pi;
W(W<-pi) = W(W<-pi)+ 2*pi;
figure(9), clf;
figure(9),subplot(2,1,1), semilogx(freq_bode2,20*log10(abs(Ghat)),'b'),xlabel('Freq(Hz)'), ylabel('Ghat(dB)vsGmodel(dB)'), hold on;
figure(9),subplot(2,1,2), semilogx(freq_bode2,rem(phase(Ghat),2*pi),'b'),xlabel('Freq(Hz)'), ylabel('phase(rad)vsphasemodel(rad)'), hold on;
figure(9),subplot(2,1,1), semilogx(freq_bode2,20*log10(abs(H)),'r'), hold on;
figure(9),subplot(2,1,2), semilogx(freq_bode2,W,'r'), hold on;